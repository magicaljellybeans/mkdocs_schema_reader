                    ...
                    with open(os.path.join(root, file)) as f:
                        ...
                        if any(x in data for x in schema_syntax):
                            ...
                            with open(path, "w") as md:
                                ...
                                # Add to Files object 
                                mkdocs_file = File(f"schema/{file[:-5]}.md", f"{os.getcwd()}/site", config["site_dir"], config["use_directory_urls"])
                                files.append(mkdocs_file)

                                # Add to schema list
                                schema_list.append({f"{mkdocs_file.name}": f"{mkdocs_file.src_path}"})
